...
A core concept to understand is that each user has _their own_ version of a symmetric key, but everyone has the _same_ symmetric key. To read more about
...
would help identify both users and project names. We are well aware of potential namespace clashes, so sit tight!
:::
