...
// TestRegisterOverwrite_FileFlag tests overwrite behavior with --file flag.
func TestRegisterOverwrite_FileFlag(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "kanuka-test-register-file-overwrite-*")
	if err != nil {
		t.Fatalf("Failed to create temp directory: %v", err)
	}
	defer os.RemoveAll(tempDir)
	...
	// Create a custom public key file.
	customKeyFile := filepath.Join(tempUserDir, "custom_user.pub")
	targetUserKeyPair := generateOverwriteTestKeyPair(t)
	...
	// First, register the user with --file.
	_, err = shared.CaptureOutput(func() error {
		cmd.ResetGlobalState()
		testCmd := shared.CreateTestCLIWithArgs("register", []string{"--file", customKeyFile}, nil, nil, true, false)
		return testCmd.Execute()
	})
	if err != nil {
		t.Fatalf("First registration should succeed: %v", err)
	...
	// Now register again with --force (user already has access).
	output, err := shared.CaptureOutput(func() error {
		cmd.ResetGlobalState()
		testCmd := shared.CreateTestCLIWithArgs("register", []string{"--file", customKeyFile, "--force"}, nil, nil, true, false)
		return testCmd.Execute()
	})
	...
	// Verify output says "access has been updated".
	if !strings.Contains(output, "access has been updated") {
		t.Errorf("Output should contain 'access has been updated' for existing user with --force, got: %s", output)
	}

	// Verify output says "Files updated".
	if !strings.Contains(output, "Files updated") {
		t.Errorf("Output should contain 'Files updated' for existing user, got: %s", output)
	}
}
