...
// TestRegisterDryRun_WithFileFlag tests that --dry-run with --file shows preview without creating files.
func TestRegisterDryRun_WithFileFlag(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "kanuka-test-register-dry-file-*")
	if err != nil {
		t.Fatalf("Failed to create temp directory: %v", err)
	}
	defer os.RemoveAll(tempDir)
	...
	// Run register with --file and --dry-run.
	output, err := shared.CaptureOutput(func() error {
		cmd.ResetGlobalState()
		testCmd := shared.CreateTestCLIWithArgs("register", []string{"--file", customKeyFile, "--dry-run"}, nil, nil, true, false)
		return testCmd.Execute()
	})
